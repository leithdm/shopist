...
def counter(request):
    ...
    # condition to check if admin is part of the requested path.
    ...
    # otherwise we will calculate the item count from the cart
        ...
        try:
            ...
            # then find the corresponding cartItems, with a [:1]
            cart_items = CartItem.objects.all().filter(cart=cart[:1])
            # loop through each cartItem and get its qty
            for cart_item in cart_items:
                item_count += cart_item.quantity
        # if there are no cartItems, the item count should be 0
